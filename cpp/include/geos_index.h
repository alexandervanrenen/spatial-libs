...
void build_strtree(	std::unique_ptr<geos::index::strtree::STRtree>* geos_strtree, std::vector<geos::geom::Geometry*>& geometry,	std::vector<int*> geom_id) {
...
void build_strtree(std::unique_ptr<geos::index::strtree::STRtree>* geos_strtree, std::vector<geos::geom::Geometry*>& geometry) {
	for (unsigned int i = 0; i < geometry.size(); i++)
...
void build_quadtree(std::unique_ptr<geos::index::quadtree::Quadtree>* geos_quadtree, std::vector<geos::geom::Geometry*>& geometry, std::vector<int*> geom_id) {
	for (unsigned int i = 0; i < geometry.size(); i++) {
...
void build_quadtree(std::unique_ptr<geos::index::quadtree::Quadtree>* geos_quadtree, std::vector<geos::geom::Geometry*>& geometry) {
	for (unsigned int i = 0; i < geometry.size(); i++) {
...
void empty_strtree(std::unique_ptr<geos::index::strtree::STRtree>* geos_strtree, std::vector<geos::geom::Geometry*>& geometry, std::vector<int*> pg) {
	for (unsigned int i = 0; i < geometry.size(); i++) {
		(*geos_strtree)->remove(geometry[i]->getEnvelopeInternal(),reinterpret_cast<void*>(pg[i]));
	}
}

void empty_strtree(std::unique_ptr<geos::index::strtree::STRtree>* geos_strtree, std::vector<geos::geom::Geometry*>& geometry) {
	for (unsigned int i = 0; i < geometry.size(); i++)
		(*geos_strtree)->remove(geometry[i]->getEnvelopeInternal(),reinterpret_cast<void*>(geometry[i]));
}

void empty_quadtree(std::unique_ptr<geos::index::quadtree::Quadtree>* geos_quadtree, std::vector<geos::geom::Geometry*>& geometry, std::vector<int*> pg) {
	for (unsigned int i = 0; i < geometry.size(); i++) {
		(*geos_quadtree)->remove(geometry[i]->getEnvelopeInternal(),reinterpret_cast<void*>(pg[i]));
	}
}

void empty_quadtree(std::unique_ptr<geos::index::quadtree::Quadtree>* geos_quadtree, std::vector<geos::geom::Geometry*>& geometry) {
	for (unsigned int i = 0; i < geometry.size(); i++) {
