...
static uint64_t pointindex_range_query(std::unique_ptr<S2PointIndex<int>>& index, S2LatLngRect& rectangle, S2Points& points) {
...
static std::vector<unsigned> point_in_polygon_query(std::unique_ptr<MutableS2ShapeIndex>& index, S2Polygons& polygons, S2Points& points) {
