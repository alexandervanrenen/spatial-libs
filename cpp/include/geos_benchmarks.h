...
auto strtree_range = [](benchmark::State& state, int dataset_size, std::string selectivity) {
	GEOSGeometry points;
	...
	if (state.thread_index == 0) {
		...
		get_ranges(target_ranges, selectivity);
...
auto quadtree_range = [](benchmark::State& state, int dataset_size, std::string selectivity) {
	GEOSGeometry points;
	GEOSGeometry target_ranges;
...
auto strtree_distance = [](benchmark::State& state, int dataset_size, std::string selectivity) {
	...
	if (state.thread_index == 0) {
		get_points(points, dataset_size);
		get_distances(target_points, target_ranges, distances, selectivity);
		build_strtree(&strtree, points.geometry, points.id);
		itr = 0;
	}

	while (state.KeepRunning ()) {
		uint64_t res_size = strtree_distance_query(*(strtree.get()), target_points.geometry[itr], target_ranges.geometry[itr], points, distances[itr]);
		benchmark::DoNotOptimize(res_size);
		itr = (itr + 1) % target_ranges.geometry.size();
	}
...
auto quadtree_distance = [](benchmark::State& state, int dataset_size, std::string selectivity) {
	GEOSGeometry points, target_ranges, target_points;
	std::vector<double> distances;
	std::unique_ptr<geos::index::quadtree::Quadtree> quadtree = std::make_unique<geos::index::quadtree::Quadtree>();
	int itr;
	...
	while (state.KeepRunning ()) {
		uint64_t res_size = quadtree_distance_query(*(quadtree.get()), target_points.geometry[itr], target_ranges.geometry[itr], distances[itr]);
		benchmark::DoNotOptimize(res_size);
		itr = (itr + 1) % target_ranges.geometry.size();
	}
...
auto strtree_distance_own = [](benchmark::State& state, int dataset_size, std::string selectivity) {
	GEOSGeometry points, target_ranges, target_points;
	std::vector<double> distances;
	std::unique_ptr<geos::index::strtree::STRtree> strtree = std::make_unique<geos::index::strtree::STRtree>();
	int itr;
	...
	while (state.KeepRunning ()) {
		uint64_t res_size = strtree_distance_query_own(*(strtree.get()), target_points.geometry[itr], target_ranges.geometry[itr], distances[itr]);
		benchmark::DoNotOptimize(res_size);
		itr = (itr + 1) % target_ranges.geometry.size();
	}
...
auto quadtree_distance_own = [](benchmark::State& state, int dataset_size, std::string selectivity) {
	GEOSGeometry points, target_ranges, target_points;
	std::vector<double> distances;
	std::unique_ptr<geos::index::quadtree::Quadtree> quadtree = std::make_unique<geos::index::quadtree::Quadtree>();
	int itr;
	...
	while (state.KeepRunning ()) {
		uint64_t res_size = quadtree_distance_query_own(*(quadtree.get()), target_points.geometry[itr], target_ranges.geometry[itr], distances[itr]);
		benchmark::DoNotOptimize(res_size);
		itr = (itr + 1) % target_ranges.geometry.size();
	}
...
auto strtree_on_points = [](benchmark::State& state, int dataset_size, std::string selectivity) {
...
auto quadtree_on_points = [](benchmark::State& state, int dataset_size, std::string selectivity) {
	GEOSGeometry points, polygons;
	GEOSPreparedGeometry prepared_polygons;
	std::unique_ptr<geos::index::quadtree::Quadtree> quadtree = std::make_unique<geos::index::quadtree::Quadtree>();
...
auto strtree_on_polygons = [](benchmark::State& state, int dataset_size, std::string selectivity) {
	GEOSGeometry points, polygons;
	GEOSPreparedGeometry prepared_polygons;
	std::unique_ptr<geos::index::strtree::STRtree> strtree = std::make_unique<geos::index::strtree::STRtree>();
...
auto quadtree_on_polygons = [](benchmark::State& state, int dataset_size, std::string selectivity) {
	GEOSGeometry points, polygons;
	GEOSPreparedGeometry prepared_polygons;
	std::unique_ptr<geos::index::quadtree::Quadtree> quadtree = std::make_unique<geos::index::quadtree::Quadtree>();
