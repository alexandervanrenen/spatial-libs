...
static void load_distance_query_points(std::vector<S2Point>& query_points, std::vector<double>& distances, const char* fileName) {
	ReadFile file;
	file.read_whole_file(fileName);
...
static void get_points(S2Points& points, int dataset_size) {
	std::string input_points_file = get_project_directory() + "/resources/datasets/raw/binary/cpp/" + std::to_string(dataset_size) + "M_rides.bin";
	load_s2_points_binary(points, input_points_file.c_str());
}

static void get_polygons(S2Polygons& polygons, std::string selectivity) {
	std::string input_polygons_file = get_project_directory() + "/resources/datasets/neighborhoods/raw/polygons_" + selectivity + ".csv";
	load_s2_neighborhoods(polygons, input_polygons_file.c_str());
}

static void get_rectangles(std::vector<S2LatLngRect>& rects, std::string selectivity) {
	std::string input_rects_file = get_project_directory() + "/resources/query_datasets/raw/taxi_range_" + selectivity + ".csv";
	load_rectangles(rects, input_rects_file.c_str());
}

static void get_distances(std::vector<S2Point>& query_points, std::vector<double>& distances, std::string selectivity) {
	std::string input_distance_file = get_project_directory() + "/resources/query_datasets/raw/taxi_distance_" + selectivity + ".csv";
