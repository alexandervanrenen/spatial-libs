...
uint64_t GetFileLength(const std::string &file_name) {
	...
	if (fileFD < 0) {
		std::cout << "Unable to open file" << std::endl; // You can check errno to see what happend
		throw;
	}
	if (fcntl(fileFD, F_GETFL) == -1) {
		std::cout << "Unable to call fcntl on file" << std::endl; // You can check errno to see what happend
		throw;
	}
	struct stat st;
	fstat(fileFD, &st);
