...
static void load_distances(GEOSGeometry& target_points, GEOSGeometry& target_ranges, std::vector<double>& distances, const char* fileName) {
	ReadFile file;
	file.read_whole_file(fileName);
	...
	while (fbegin != fend) {
		...
		geos::geom::Envelope rectangle(geos::geom::Coordinate(x - distance, y - distance), geos::geom::Coordinate(x + distance, y + distance));
...
static void get_ranges(GEOSGeometry& target_ranges, std::string selectivity) {
	std::string input_range_file = get_project_directory() + "/resources/query_datasets/projected/taxi_range_" + selectivity + ".csv";
...
static void get_points(GEOSGeometry& points, int dataset_size) {
	std::string input_points_file = get_project_directory() + "/resources/datasets/projected/binary/cpp/" + std::to_string(dataset_size) + "M_rides.bin";
	load_geos_points_binary(points, input_points_file);
}

static void get_polygons(GEOSGeometry& polygons, std::string selectivity) {
	std::string input_polygons_file = get_project_directory() + "/resources/datasets/neighborhoods/projected/polygons_" + selectivity + ".csv";
