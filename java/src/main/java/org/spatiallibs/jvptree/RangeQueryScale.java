...
public class RangeQueryScale {
	...
	public void setup() {
		...
		ranges = Utilities.getEnvelopes(System.getProperty("user.dir") + "/resources/query_datasets/projected/taxi_range_" + selectivity + ".csv");
	...
	public void benchmark(Blackhole blackhole) {
		List<Double> range = ranges.get(idx);
		List<SimpleCartesianPoint> result = Queries.rangeQuery(vptree, range);
		blackhole.consume(result);
		idx = (idx + 1) % ranges.size();
	}
}
